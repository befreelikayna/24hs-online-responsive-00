...
export const ContentPanel = ({ activeSection, isLoggedIn = false, onDemoLogin }: ContentPanelProps) => {
  ...
  const getStateIcon = () => {
    ...
    return isFullscreen ? (
      <Minimize2 className="w-5 h-5 text-[#00b4d8] cursor-pointer hover:text-[#90e0ef] transition-colors" />
    ) : (
      <Maximize2 className="w-5 h-5 text-[#00b4d8] cursor-pointer hover:text-[#90e0ef] transition-colors" />
    );
  };
